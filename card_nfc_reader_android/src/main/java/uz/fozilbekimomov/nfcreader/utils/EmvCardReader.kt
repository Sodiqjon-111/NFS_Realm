package uz.fozilbekimomov.nfcreader.utils

import android.nfc.tech.IsoDep
import uz.fozilbekimomov.nfcreader.model.ApplicationFileLocator
import uz.fozilbekimomov.nfcreader.model.EmvCard
import uz.fozilbekimomov.nfcreader.model.commandhelpers.EmvCardScheme
import uz.fozilbekimomov.nfcreader.model.commandhelpers.EmvTags
import uz.fozilbekimomov.nfcreader.model.commandhelpers.StatusEnum
import uz.fozilbekimomov.nfcreader.model.commandhelpers.StatusEnum.Companion.isEqual
import uz.fozilbekimomov.nfcreader.model.constructByteValue
...
class EmvCardReader(private val isoDep: IsoDep) {
    ...
    private fun parseFCIProprietaryTemplate(inputData: ByteArray?): ByteArray? { // Get SFI
        ...
        if (data != null) {
            ...
            data = transceive(
                CommandApduCreator.create(
                    CommandApduCreator.CommandType.READ_RECORD,
                    sfi,
                    sfi shl 3 or 4,
                    0
                )
    ...
    private fun readPpseDirectory(): Boolean {
        var data = transceive(
            CommandApduCreator.create(
                CommandApduCreator.CommandType.SELECT,
                PPSE_DIRECTORY,
                0
            )
        )
    ...
    private fun getGetProcessingOptions(pdolData: ByteArray?): ByteArray? { // List Tag and length from PDOL
        ...
        return transceive(
            CommandApduCreator.create(
                CommandApduCreator.CommandType.GPO,
                out.toByteArray(),
                0
            )
        )
